...
export const calculateProfileComplete = async (candidateId) => {
  ...
  let score = 0;
  if (candidate.firstName && candidate.lastName && candidate.city && candidate.state && candidate.mobile) score += 20;
  ...
  await db.candidate.update({ where: { id: candidateId }, data: { profileComplete: score } });
  return score;
};
