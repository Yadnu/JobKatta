...
export const activateCandidatePlan = async (candidateId, planKey, subscriptionId) => {
  ...
  await db.candidate.update({ where: { id: candidateId }, data: { planType: plan.planType, planExpiresAt: expiresAt } });
  await db.subscription.update({ where: { id: subscriptionId }, data: { status: 'paid', startsAt: new Date(), expiresAt } });
};
...
export const activateEmployerPlan = async (employerId, planKey, subscriptionId) => {
  ...
  if (!plan) throw new Error('Invalid plan');
  const expiresAt = new Date(Date.now() + plan.duration * 24 * 60 * 60 * 1000);
  await db.employer.update({ where: { id: employerId }, data: { planType: plan.planType, planExpiresAt: expiresAt, activeJobLimit: plan.activeJobs } });
  await db.subscription.update({ where: { id: subscriptionId }, data: { status: 'paid', startsAt: new Date(), expiresAt } });
};
...
export const checkEmployerJobLimit = async (employerId) => {
  ...
  const activeJobs = await db.job.count({ where: { employerId, status: { in: ['ACTIVE', 'PENDING_APPROVAL'] } } });
...
export const checkCandidateAppLimit = async (candidateId) => {
  ...
  if (needsReset) {
    await db.candidate.update({ where: { id: candidateId }, data: { appCountThisMonth: 0, appCountReset: now } });
    return { allowed: true };
  }
  if (candidate.appCountThisMonth >= 5) {
    return { allowed: false, reason: 'Free plan limit: 5 applications per month. Upgrade to Premium for unlimited.' };
  }
  return { allowed: true };
};
