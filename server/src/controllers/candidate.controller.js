...
export const updateProfile = catchAsync(async (req, res) => {
  ...
  const allowedFields = [
    ...
    'addressLine1', 'city', 'state', 'pincode', 'portfolioUrl',
    'linkedinUrl', 'githubUrl', 'bio', 'isFresher', 'totalExperienceYrs', 'openToWork',
  ];
...
export const updateProfileStep = catchAsync(async (req, res) => {
  ...
  switch (parseInt(stepNumber)) {
    case 1:
      ...
      break;
    case 3:
      data = { isFresher: req.body.isFresher, totalExperienceYrs: req.body.totalExperienceYrs };
      break;
    case 4:
      data = {
        ...
        preferredEmpType: req.body.preferredEmpType,
  ...
  const profileComplete = await calculateProfileComplete(candidate.id);
  res.json({ success: true, message: `Step ${stepNumber} saved`, data: { profileComplete } });
});
...
export const getProfileCompleteness = catchAsync(async (req, res) => {
  ...
  res.json({ success: true, message: 'Profile completeness', data: { score } });
});
...
export const addEducation = catchAsync(async (req, res) => {
  ...
  const education = await db.education.create({
    data: {
      ...
      degree: xss(req.body.degree),
      fieldOfStudy: req.body.fieldOfStudy ? xss(req.body.fieldOfStudy) : null,
      institution: xss(req.body.institution),
      board: req.body.board ? xss(req.body.board) : null,
      percentage: req.body.percentage,
      startYear: parseInt(req.body.startYear),
      endYear: req.body.endYear ? parseInt(req.body.endYear) : null,
      isCurrently: req.body.isCurrently || false,
    },
  });
...
export const updateEducation = catchAsync(async (req, res) => {
  ...
  const updated = await db.education.update({
    ...
    data: {
      degree: req.body.degree ? xss(req.body.degree) : undefined,
      fieldOfStudy: req.body.fieldOfStudy ? xss(req.body.fieldOfStudy) : undefined,
      institution: req.body.institution ? xss(req.body.institution) : undefined,
      board: req.body.board ? xss(req.body.board) : undefined,
      percentage: req.body.percentage,
      startYear: req.body.startYear ? parseInt(req.body.startYear) : undefined,
      endYear: req.body.endYear ? parseInt(req.body.endYear) : undefined,
...
export const addExperience = catchAsync(async (req, res) => {
  ...
  const experience = await db.experience.create({
    data: {
      ...
      city: req.body.city ? xss(req.body.city) : null,
...
export const updateExperience = catchAsync(async (req, res) => {
  ...
  if (!exp) throw new AppError('Experience record not found', 404);

  const updated = await db.experience.update({
    where: { id: req.params.id },
    data: {
      jobTitle: req.body.jobTitle ? xss(req.body.jobTitle) : undefined,
      companyName: req.body.companyName ? xss(req.body.companyName) : undefined,
      city: req.body.city ? xss(req.body.city) : undefined,
      employmentType: req.body.employmentType,
...
export const toggleOpenToWork = catchAsync(async (req, res) => {
  ...
  const updated = await db.candidate.update({
    where: { id: candidate.id },
    data: { openToWork: !candidate.openToWork },
  });
...
export const createAlert = catchAsync(async (req, res) => {
  ...
  const alert = await db.emailAlert.create({
    data: {
      candidateId: candidate.id,
...
export const updateAlert = catchAsync(async (req, res) => {
  ...
  const updated = await db.emailAlert.update({
    where: { id: req.params.id },
    data: {
      keywords: req.body.keywords,
      city: req.body.city,
