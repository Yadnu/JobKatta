...
export const listJobs = catchAsync(async (req, res) => {
  ...
  const [jobs, total] = await Promise.all([
    db.job.findMany({
      where,
      include: {
        employer: { select: { companyName: true, logoUrl: true, city: true, industry: true } },
        skills: { include: { skill: true } },
      },
      orderBy: [{ isPriority: 'desc' }, { isFeatured: 'desc' }, { createdAt: 'desc' }],
      skip,
      take: limit,
    }),
    db.job.count({ where }),
...
export const getJob = catchAsync(async (req, res) => {
  const job = await db.job.findFirst({
    ...
    include: {
      employer: {
        select: {
          companyName: true, logoUrl: true, city: true, state: true,
  ...
  if (req.user) {
    ...
    if (candidate) {
      await db.recentlyViewedJob.upsert({
        where: { candidateId_jobId: { candidateId: candidate.id, jobId: job.id } },
        update: { viewedAt: new Date() },
        create: { candidateId: candidate.id, jobId: job.id },
      });
    }
  }
...
export const applyToJob = catchAsync(async (req, res) => {
  ...
  if (candidate.profileComplete < 50) {
    throw new AppError('Complete at least 50% of your profile before applying', 400);
  }
  ...
  if (candidate.user?.email) {
    await sendApplicationSubmittedEmail(candidate.user.email, {
